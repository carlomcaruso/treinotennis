...
def baixa_do_dropbox(destino):
    import requests
    faltando = [v for v in ('DROPBOX_APP_KEY','DROPBOX_APP_SECRET','DROPBOX_REFRESH_TOKEN')
                if not os.environ.get(v)]
    if faltando:
        raise SystemExit("ERRO: secrets ausentes no GitHub: " + ', '.join(faltando))

    # .strip() protege contra espaço ou quebra de linha colados junto no secret
    app_key    = os.environ['DROPBOX_APP_KEY'].strip().strip('"\'')
    app_secret = os.environ['DROPBOX_APP_SECRET'].strip().strip('"\'')
    refresh    = os.environ['DROPBOX_REFRESH_TOKEN'].strip().strip('"\'')
    ...
    if tk.status_code != 200:
        try:
            ...
            erro = {'raw': tk.text[:400]}
        codigo = str(erro.get('error', erro))
        print(f"Dropbox respondeu {tk.status_code}: {json.dumps(erro, ensure_ascii=False)}")
        if 'invalid_grant' in codigo:
            raise SystemExit(
                "DIAGNÓSTICO: o refresh token não vale mais (foi revogado ou está errado).\n"
                "  Gere outro: repita os passos 4 e 5 e atualize o secret DROPBOX_REFRESH_TOKEN.")
        if 'invalid_client' in codigo or tk.status_code == 401:
            raise SystemExit(
                "DIAGNÓSTICO: App key ou App secret incorretos.\n"
                "  Confira os dois na aba Settings do app em dropbox.com/developers/apps.")
        raise SystemExit("Falha na autenticação com o Dropbox — veja a resposta acima.")
    ...
    if r.status_code != 200:
        print(f"Dropbox respondeu {r.status_code} ao baixar: {r.text[:400]}")
        if 'path/not_found' in r.text:
            raise SystemExit(
                f"DIAGNÓSTICO: planilha não encontrada em '{CAMINHO_DROPBOX}'.\n"
                "  Confira o caminho exato (maiúsculas e acentos) no seu Dropbox.")
        if 'missing_scope' in r.text:
            raise SystemExit(
                "DIAGNÓSTICO: faltam permissões no app do Dropbox.\n"
                "  Marque files.metadata.read e files.content.read na aba Permissions,\n"
                "  clique em Submit, e gere um refresh token NOVO (permissão antiga não vale).")
        raise SystemExit("Falha ao baixar a planilha — veja a resposta acima.")
